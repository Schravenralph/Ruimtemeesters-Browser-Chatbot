...
| #   | Decision                                                                  | Detail                                                                                                                                                                                                                                                                                                    |
| --- | ------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | **Three persona models, slugs match canon**                               | `RO-Assistent`, `Juridisch-Assistent`, `Commercieel-Assistent` (matching the LiteLLM routing key + OWUI Model row id + system-prompt persona name set in compose/seed via #82). Existing model entries that don't match are stale (see Platform ADR-0011 cleanup list) and get removed in a follow-up PR. |
...
| -------------------------- | ----------------------- | ----------------- | -------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| **bopa**                   | Shipped                 | ro-assistent      | juridisch-assistent (toetsing onderdelen)                      | All 18 — BOPA Phase 3 (Beleid) and Phase 4 (Omgevingsaspecten) scan multiple thema's |
| **beleidsscan**            | Planned (active design) | ro-assistent      | juridisch-assistent (for Omgevingsplan thema)                  | 1 of 18 per invocation; cross-references for closely related thema's (e.g. #1 ↔ #18) |
...
| **onderbouwing-schrijver** | Planned                 | ro-assistent      | juridisch-assistent (for omgevingsplan wijziging onderbouwing) | All 18 (writes from scan outputs)                                                    |
| **visie-kader-analyse**    | Planned                 | ro-assistent      | —                                                              | All 18 (analyses incoming visies, maps to thema's)                                   |

### Persona ownership consequence

- **RO Assistent** invokes all seven skills; primary owner.
- **Juridisch Assistent** invokes `beleidsscan` when targeting Omgevingsplan (thema #15 per P-0009), `mer-plichttoets` when the procedure-formality is the focus, `onderbouwing-schrijver` when writing a juridische onderbouwing, and `bopa` for legal-toetsing aspects.
- **Commercieel Assistent** currently invokes none — consumes scan _outputs_ via `Document-Generator` or direct file reads, but does not drive scans. A future commercial-side skill (e.g. `tender-go-no-go`) is a candidate; tracked as out-of-scope here.
...
| --------------------- | ---------------------------------------------------------------------------------------------------------- |
| ro-assistent          | rm-geoportaal (all), rm-databank (search/tagging), rm-memory, rm-aggregator, rm-document-generator         |
| juridisch-assistent   | rm-databank (search/tagging), rm-wetten, rm-nieuwsbrief (jurisprudentie), rm-memory, rm-document-generator |
| commercieel-assistent | rm-riens, rm-opdrachten-scanner, rm-sales-predictor, rm-memory, rm-crm, rm-mailchimp                       |
...
- A future `tender-go-no-go` or similar commercial-side skill for Commercieel Assistent — candidate, not committed.
