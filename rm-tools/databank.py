...
class Tools:
    class Valves(BaseModel):
        ...
        api_key: str = Field(
            default="",
            description="Service API key (X-API-Key header) for internal access",
        )
    ...
    async def search_beleidsdocumenten(self, query: str, location: str = "", document_type: str = "", limit: int = 10, __user__: dict = {}) -> str:
        """
        ...
        async with httpx.AsyncClient(timeout=self.valves.timeout) as client:
            resp = await client.get(f"{self.valves.databank_api_url}/api/search", params=params, headers=self._headers())
            resp.raise_for_status()
            return resp.text

    async def get_knowledge_graph(self, entity_id: str = "", __user__: dict = {}) -> str:
        """
        ...
        async with httpx.AsyncClient(timeout=self.valves.timeout) as client:
            if entity_id:
                resp = await client.get(f"{self.valves.databank_api_url}/api/knowledge-graph/entity/{entity_id}", headers=self._headers())
            else:
                resp = await client.get(f"{self.valves.databank_api_url}/api/knowledge-graph", params={"limit": 50}, headers=self._headers())
            resp.raise_for_status()
            return resp.text

    async def get_document(self, document_id: str, __user__: dict = {}) -> str:
        """
        ...
        """
        async with httpx.AsyncClient(timeout=self.valves.timeout) as client:
            resp = await client.get(f"{self.valves.databank_api_url}/api/canonical-documents/{document_id}", headers=self._headers())
            resp.raise_for_status()
            return resp.text

    async def list_queries(self, __user__: dict = {}) -> str:
        """
        List the user's beleidsscan queries (policy scan searches).
        :return: List of queries with their status and results
        """
        async with httpx.AsyncClient(timeout=self.valves.timeout) as client:
            resp = await client.get(f"{self.valves.databank_api_url}/api/queries", headers=self._headers())
            resp.raise_for_status()
            return resp.text

    async def create_query(self, search_text: str, location: str = "", __user__: dict = {}) -> str:
        """
        ...
        async with httpx.AsyncClient(timeout=self.valves.timeout) as client:
            resp = await client.post(f"{self.valves.databank_api_url}/api/queries", json=body, headers=self._headers())
            resp.raise_for_status()
            return resp.text
